...
import { colorInfoLabel, hexToLab, labToOklch, visibleSwatchLab } from "../src/color-utils.js";

test("colorInfoLabel derives LCH from visible hex before internal lab fallback", () => {
  assert.equal(colorInfoLabel("#000003", [0, 7.4, 0]), "#000003 · LCH 4.4 3.0 —");
...
test("labToOklch preserves raw hue for low-chroma colors", () => {
  const [, blueC, blueH] = labToOklch(hexToLab("#c7d5ef"));
  const [, peachC, peachH] = labToOklch(hexToLab("#f0cac6"));
  assert.ok(blueC > 0);
  assert.ok(peachC > 0);
  assert.notEqual(Math.round(blueH * 180 / Math.PI), 0);
  assert.notEqual(Math.round(peachH * 180 / Math.PI), 0);
  assert.ok(Math.abs(blueH - peachH) > 1);
});
