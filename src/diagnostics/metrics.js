...
export function diagnosticsSignature({imageData, records = [], entries = [], config = {}, includeCycleOffset = false} = {}) {
  ...
  return [
    ...
    config.ditherSourceGuardEnabled ? 1 : 0,
    Number(config.ditherSourceGuardAmount || 0).toFixed(3),
    Number(config.ditherSourceGuardMinGain || 0).toFixed(3),
    Number(config.ditherSourceGuardFlatThreshold || 0).toFixed(3),
    config.blendAmount,
