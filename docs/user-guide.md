...
- **Pixel-art post-process:** despeckle, a 3×3 mode filter with 1–4 passes. It runs at art-pixel granularity so it respects `pixelBlockSize`. The diagnostic overlay short-circuits the post-process pipeline, so swatch and difference views always reflect raw palette output.
